...
gene_mart <- read.table("mart_export_GRCh38.txt", header = T, sep = "\t")
...
data <- read.table("../ARCH4S_retrieval/GSE13090_expression_matrix.tsv", header = T, sep = "\t")
# Some genes are duplicated, so add counts - CHECK IF THIS IS CORRECT
# Order of genes will be shuffled a bit
data <- process_duplicated_rows(data, "X")
# Load metadata
metadata <- read.table("GSE130970_Hoang_etal/METADATA.txt", header = T, sep = ",")
...
data <- read.table("../ARCH4S_retrieval/GSE135251_expression_matrix.tsv", header = T, sep = "\t")
# Some genes are duplicated, so add counts - CHECK IF THIS IS CORRECT
# Order of genes will be shuffled a bit
data <- process_duplicated_rows(data, "X")
# Load metadata
metadata <- read.table("GSE135251_Govaere_etal/METADATA.txt", header = T, sep = ",")
...
data <- read.table("GSE166256_Wang_etal_liverchip/GSE166256_kallisto_counts.tsv", header = T, sep = "\t")
rownames(data) <- data$Gene
data <- data[,-1]
# Extract metadata
metadata <- read.table("GSE166256_Wang_etal_liverchip/METADATA_Wangetal2020.txt", header = T, sep = "\t")
...
data <- read.table("GSE168285_Kostrzewski_etal/GSE168285_raw_counts.txt", header = T, sep = "\t")
colnames(data)[1] <- "X"
data <- translate_genes(data, gene_mart)
# Load metadata
metadata <- read.table("GSE168285_Kostrzewski_etal/GSE168285_sample_meta_data.txt", header = T, sep = "\t")
...
exp_matrix <- read.table("GSE168285_Kostrzewski_etal/design_matrix_MPS.txt", header = T, sep = "\t")
...
data <- read.table("../ARCH4S_retrieval/GSE89063_expression_matrix.tsv", header = T, sep = "\t")
data <- process_duplicated_rows(data, "X")
# Load metadata
metadata <- read.table("../ARCH4S_retrieval/sample_data_Feaver.txt", header = T, sep = "\t")
exp_factors <- "media"
