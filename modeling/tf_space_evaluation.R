# LIV2TRANS in TF-activity space (high k/p regime)
# This version adds Tanimoto similarity and GSEA-based distance at
# multiple top-N values (5, 10, 20, 30, 50, 75, 100) for both TCs
# and extra LVs. Cosine-similarity figures are unchanged; new
# figures are saved separately.
...
library(matrixStats)
library(decoupleR)
library(fgsea)        # GSEA-based distance
...
N_RANDOM_REPS    <- 50
TOP_N_CONDITIONS <- 6
TOP_N_VALUES     <- c(5, 10, 20, 30, 50, 75, 100)   # for TF space
FGSEA_NPERM      <- 100                              # we only need ES, not p
...
network <- tryCatch(
  decoupleR::get_dorothea(levels = c("A", "B"), organism = "human"),
  error = function(e) {
    cat("  decoupleR::get_dorothea failed; trying OmnipathR fallback\n")
    OmnipathR::dorothea_interactions(levels = c("A", "B"))
  }
)
cat("  DoRothEA regulon loaded:", nrow(network), "interactions,",
    length(unique(network$source)), "TFs\n")

run_tf_inference <- function(expr_mat) {
...
TF_h_raw <- run_tf_inference(Xh_gene)
TF_m_raw <- run_tf_inference(Xm_gene)

common_tfs <- intersect(colnames(TF_h_raw), colnames(TF_m_raw))
TF_h <- TF_h_raw[, common_tfs, drop = FALSE]
TF_m <- TF_m_raw[, common_tfs, drop = FALSE]
TF_m <- TF_m[rownames(metadata_m), , drop = FALSE]
ref_meta <- data_list[[ref_dataset]]$metadata
# TF_h     <- TF_h[rownames(ref_meta), , drop = FALSE]

p_TF <- ncol(TF_h)
cat("  TF activities computed:", p_TF, "TFs;",
    nrow(TF_h), "human samples;", nrow(TF_m), "MPS samples\n")

Xh_TF <- scale(TF_h, center = TRUE, scale = FALSE)
Xm_TF <- scale(TF_m, center = TRUE, scale = FALSE)
...
# ---------------------------------------------------------------
# NEW: Tanimoto + GSEA-based distance helpers
# ---------------------------------------------------------------
# Tanimoto/Jaccard between top-N feature sets of two vectors
# (both ranked by absolute loading).
...
# Symmetric GSEA-based ES between two vectors using top-N as set
# and |loading| of the other vector as the ranking statistic.
# Returns mean(|ES_AB|, |ES_BA|) and 1 - mean(|ES|) as a distance.
gsea_topN <- function(vA, vB, top_n) {
  ...
  run_fgsea <- function(set_top, stats_vec) {
    # Ranking stats must be unique; break ties with tiny jitter.
...
# For each matched (A_col, B_col) pair from matched_cosine, compute
# Tanimoto and GSEA-distance at each top_n. Returns long-format df:
# rep, A_col, B_col, top_n, tanimoto, gsea_es, gsea_distance.
...
# ===============================================================
# PART 4: REAL LIV2TRANS RUN IN TF SPACE
# ===============================================================
cat("\n=== Real LIV2TRANS in TF space ===\n")

Wm_TF <- compute_Wm_from_matrix(TF_m, real_groups)
k_TF  <- ncol(Wm_TF)
cat("  k_TF =", k_TF, "  p_TF =", p_TF,
    "  k/p =", round(k_TF/p_TF, 4), "\n")

liv2trans_real <- liv2trans_run(Xh_TF, Yh, Xm_TF, Wm_TF, LV_number = 2)

Wm_opt_real <- liv2trans_real$W_opt
rownames(Wm_opt_real) <- rownames(Wm_TF)
colnames(Wm_opt_real) <- paste0("LV_extra", seq_len(ncol(Wm_opt_real)))

W_TC_real <- as.matrix(liv2trans_real$W_translatable)
rownames(W_TC_real) <- rownames(Wm_TF)
...
rownames(phi_real) <- rownames(Wm_TF)

alpha_real <- compute_alpha(Wm_TF, phi_real)
...
cat("    effective rank :", r_eff_real, " (k =", k_TF, ")\n")
...
# ---- (b) Random Gaussian TF matrix (full pipeline) ----
cat("\n=== Null (b): random Gaussian TF matrix  (N =", N_RANDOM_REPS, ") ===\n")
mu_TF  <- mean(TF_m);  sig_TF <- sd(TF_m)
random_TF_runs <- list()
for (i in seq_len(N_RANDOM_REPS)) {
  if (i %% 10 == 0) cat("  ", i, "/", N_RANDOM_REPS, "\n")
  tryCatch({
    TF_m_rand <- matrix(rnorm(nrow(TF_m) * p_TF, mean = mu_TF, sd = sig_TF),
                        nrow = nrow(TF_m), ncol = p_TF,
                        dimnames = dimnames(TF_m))
    Xm_TF_rand <- scale(TF_m_rand, center = TRUE, scale = FALSE)
    Wm_rand    <- compute_Wm_from_matrix(TF_m_rand, real_groups, k = k_TF)
    ...
    res_rand <- liv2trans_run(Xh_TF, Yh, Xm_TF_rand, Wm_rand, LV_number = 2)
    ...
    random_TF_runs[[i]] <- list(
      TF_m = TF_m_rand, Xm_TF = Xm_TF_rand,
...
# ---- (c) Shuffled TF table (full pipeline) ----
cat("\n=== Null (c): shuffled TF table  (N =", N_RANDOM_REPS, ") ===\n")
shuffled_TF_runs <- list()
for (i in seq_len(N_RANDOM_REPS)) {
  if (i %% 10 == 0) cat("  ", i, "/", N_RANDOM_REPS, "\n")
  tryCatch({
    TF_m_sh <- matrix(sample(as.vector(TF_m)),
                      nrow = nrow(TF_m), ncol = p_TF,
                      dimnames = dimnames(TF_m))
    Xm_TF_sh <- scale(TF_m_sh, center = TRUE, scale = FALSE)
    Wm_sh    <- compute_Wm_from_matrix(TF_m_sh, real_groups, k = k_TF)
    
    a_r <- compute_alpha(Wm_sh, phi_real)
    results_align[[length(results_align) + 1]] <- data.frame(
      source = "Shuffled TF table", rep = i,
      phenotype     = colnames(phi_real),
      r_eff         = apply(a_r, 2, effective_rank),
      concentration = apply(a_r, 2, concentration),
      rho_total     = colSums(a_r),
    ...
    res_sh <- liv2trans_run(Xh_TF, Yh, Xm_TF_sh, Wm_sh, LV_number = 2)
    ...
    if (ncol(W_TC_sh) > 0 && ncol(W_TC_real) > 0) {
      ...
      cs$source <- "Shuffled TF table"; cs$rep <- i; cs$kind <- "TC"
      results_TC[[length(results_TC) + 1]] <- cs
      
      tn <- matched_topN_metrics(W_TC_real, W_TC_sh, TOP_N_VALUES)
      if (nrow(tn) > 0) {
        tn$source <- "Shuffled TF table"; tn$rep <- i; tn$kind <- "TC"
        results_TC_topN[[length(results_TC_topN) + 1]] <- tn
      }
    }
    
    shuffled_TF_runs[[i]] <- list(
      TF_m = TF_m_sh, Xm_TF = Xm_TF_sh,
      Wm = Wm_sh, W_TC = W_TC_sh, Wm_opt = Wm_opt_sh,
      source = "Shuffled TF table"
    )
  }, error = function(e) cat("    rep", i, "FAILED:", e$message, "\n"))
}
...
reduce_and_rerun <- function(TF_m_mat, Xm_TF_mat, W_TC, label) {
  sel <- select_driving_conditions(Xm_TF_mat, real_groups, W_TC,
  ...
  Xm_sub  <- Xm_TF_mat[kept_samples, , drop = FALSE]
  groups_sub <- real_groups[kept_samples]
  Wm_sub  <- compute_Wm_from_matrix(TF_m_mat[kept_samples, , drop = FALSE],
                                    groups_sub)
  res_sub <- liv2trans_run(Xh_TF, Yh, Xm_sub, Wm_sub, LV_number = 2)
...
real_reduced <- reduce_and_rerun(TF_m, Xm_TF, W_TC_real, "real")
...
for (i in seq_along(random_TF_runs)) {
  r <- random_TF_runs[[i]]
  if (is.null(r) || is.null(r$W_TC) || ncol(r$W_TC) < 1) next
  red <- reduce_and_rerun(r$TF_m, r$Xm_TF, r$W_TC, paste0("rand_", i))
  if (is.null(red)) next
...
cat("  Reducing shuffled TF runs...\n")
for (i in seq_along(shuffled_TF_runs)) {
  r <- shuffled_TF_runs[[i]]
  if (is.null(r) || is.null(r$W_TC) || ncol(r$W_TC) < 1) next
  red <- reduce_and_rerun(r$TF_m, r$Xm_TF, r$W_TC, paste0("shuf_", i))
  if (is.null(red)) next
  cs <- matched_cosine(real_reduced$Wm_opt, red$Wm_opt)
  if (nrow(cs) > 0) {
    cs$source <- "Shuffled TF table"; cs$rep <- i; cs$kind <- "LV_extra"
    results_LVopt[[length(results_LVopt) + 1]] <- cs
    
    tn <- matched_topN_metrics(real_reduced$Wm_opt, red$Wm_opt, TOP_N_VALUES)
    if (nrow(tn) > 0) {
      tn$source <- "Shuffled TF table"; tn$rep <- i; tn$kind <- "LV_extra"
      results_LVopt_topN[[length(results_LVopt_topN) + 1]] <- tn
    }
  }
}
...
# ---- 7a: alignment metrics (unchanged) ----
...
p_reff <- ggplot(df_align, aes(x = source, y = r_eff)) +
  ...
  labs(x = "", y = "Effective rank of alignment",
       subtitle = "Red dashed = real Wm (TF space)") +
...
if (nrow(df_TC_topN) > 0) {
  ...
  p_TCtan <- ggplot(df_TC_topN, aes(x = top_n_f, y = tanimoto, fill = source)) +
    ...
    labs(x = "Top-N features", y = "Tanimoto similarity",
         fill = "",
  ...
  p_TCgsea <- ggplot(df_TC_topN, aes(x = top_n_f, y = gsea_distance, fill = source)) +
    ...
    facet_wrap(~ A_col, scales = "fixed") +
    scale_y_continuous(limits = c(0, 1)) +
    labs(x = "Top-N features", y = "GSEA-based distance  (1 - mean |ES|)",
         fill = "",
...
# ---- 7d: NEW — Tanimoto / GSEA-distance (extra LV) ----
...
save(df_align, df_TC, df_TC_topN, df_LVopt, df_LVopt_topN,
     ...
     Wm_TF, Wm_opt_real, W_TC_real, phi_real,
     real_reduced,
     TF_h, TF_m, real_groups, condition_col,
     k_TF, p_TF, TOP_N_CONDITIONS, TOP_N_VALUES,
     ref_dataset, target_dataset, dataset_tag,
     file = file.path(RESULTS_DIR,
                      paste0(dataset_tag,
                             "__tf_space_results_with_topN.RData")))

cat("\n============================================\n")
cat("Plots:   ", FIGURES_DIR, "\n")
cat("Results: ",
    file.path(RESULTS_DIR,
              paste0(dataset_tag, "__tf_space_results_with_topN.RData")),
    "\n", sep = "")
cat("============================================\n")
