...
pert_activity <- decoupleR::run_viper(Woptim, 
                                      resp_net %>% unique(),
                                      minsize = 1,
                                      verbose = TRUE)
...
pert_activity <- pert_activity %>% mutate(significant = ifelse(`p-value`<=0.05 & abs(activity)>=2.5,
                                                               perturbation,
                                                               significant))
pert_activity <- pert_activity %>% mutate(significant = ifelse(Chemical_Compound=='Ifna',
                                                               perturbation,
                                                               significant))
# if (nrow(pert_activity %>% filter(`p-value`<=0.1))<10){
#   top_10s <- pert_activity$perturbation[order(-abs(pert_activity$activity))[1:10]]
#   pert_activity <- pert_activity %>% mutate(significant = ifelse((perturbation %in% top_10s) | (`p-value`<=0.1),
#                                                                  perturbation,
#                                                                  significant))
# }
...
(ggplot(pert_activity %>% filter(condition=='V1'),aes(x=reorder(perturbation,activity),y=activity,fill = `p-value`)) + geom_point(shape=21,size=2) +
