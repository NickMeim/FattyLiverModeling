...
#       min_steat <- val_Y[3]
#       saveRDS(ind,paste0('../preprocessing/TrainingValidationData/10fold_lasso_female/train_indices_',i,'.rds'))
#     }
...
for (i in 1:num_folds){
  train_inds <- readRDS(paste0('../preprocessing/TrainingValidationData/10fold_lasso_female/train_indices_',i,'.rds'))
  ...
  saveRDS(results_folds,'../results/TransCompR_PCA_Females/all_laso_models.rds')
...
df_all <- rbind(df_all,df)
saveRDS(df_all,'../results/TransCompR_PCA_Females/df_all.rds')
...
saveRDS(df,'../results/TransCompR_PCA_Females/df_genes_human_lasso.rds')
df_all <- rbind(df_all,df)
saveRDS(df_all,'../results/TransCompR_PCA_Females/df_all_long.rds')
...
saveRDS(df,'../results/TransCompR_PCA_Females/df_performance_humanPC.rds')

### Final visualize-----------------------------------------------------------
df_all <- readRDS('../results/TransCompR_PCA_Females/df_all_long.rds')
df_performance_human <- readRDS('../results/TransCompR_PCA_Females/df_performance_humanPC.rds')
...
print(p)
ggsave('../results/TransCompR_PCA_Females/allgenes_vs_translated_performance.png',
       plot=p,
       width = 16,
       height = 8,
       units = 'in',
       dpi = 600)
ggsave('../results/TransCompR_PCA_Females/allgenes_vs_translated_mae.png',
       plot=p1,
       width = 16,
       height = 8,
       units = 'in',
...
num_folds <- 10
results <- readRDS('../results/TransCompR_PCA_Females/all_laso_models.rds')
df_res <- data.frame()
...
# saveRDS(df_res_random,'../results/TransCompR_PCA_Females/counts_random_pcs.rds')
df_res_random <- readRDS('../results/TransCompR_PCA_Females/counts_random_pcs.rds')
...
ggplot(cor_test_results_df, aes(variable1, variable2, fill = correlation))+
  ...
  theme(legend.position = 'right',
        ...
        axis.text.x = element_text(angle = 45, vjust = 1, 
                                   size = 12, hjust = 1)) + 
  ylab('') + xlab('')
ggsave('../results/TransCompR_PCA_Females/selected_pcs_correlation.png',
       width = 9,
       height = 9,
       units = 'in',
       dpi=600)
...
p_nas <- p1_nas+p2_nas
print(p_nas)
ggsave('../results/TransCompR_PCA_Females/important_pcs_vs_nas.png',
       plot = p_nas,
       width = 14,
       height = 9,
       units = 'in',
...
print(p_fibrosis)
ggsave('../results/TransCompR_PCA_Females/important_pcs_vs_fibrosis.png',
       plot = p_fibrosis,
       width = 14,
       height = 9,
       units = 'in',
...
print(p_steatosis)
ggsave('../results/TransCompR_PCA_Females/important_pcs_vs_steatosis.png',
       plot = p_steatosis,
       width = 14,
       height = 9,
       units = 'in',
...
print(p_balooning)
ggsave('../results/TransCompR_PCA_Females/important_pcs_vs_balooning.png',
       plot = p_balooning,
       width = 14,
       height = 9,
       units = 'in',
...
# keep_pcs <- unique(df_res$PC)
keep_pcs <- c('PC10','PC11','PC12','PC13','PC27','PC35','PC38','PC40','PC48')
Zb <- PCA_alldata$x
...
anov <- aov(cbind(PC10,PC11,PC12,PC13,PC27,PC35,PC38,PC40,PC48)~Background+LPS+TGFb+Cholesterol+Fructose+NPC,data=scores)
summary(anov)
...
p <- p_F + p_coeff
print(p)
ggsave('../results/TransCompR_PCA_Females/anova_stimuli.png',
       plot=p,
       height = 6,
       width = 12,
       units = 'in',
