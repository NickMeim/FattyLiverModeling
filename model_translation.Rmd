...
```{r}
Wm_rank <- rank_LV_MPS(Xh, Yh, Wh, Bh, Wm, retrain = F)
...
while (d_Ey > 1){
  ...
  Ypred <- cbind(1, Xh %*% Wm_new_basis %*% t(Wm_new_basis) %*% Wh) %*% Bh
...
Ypred2 <- cbind(1, Xh %*% Wm_new_basis %*% t(Wm_new_basis) %*% Wh) %*% Bh
...
plt_dummy <- plot_scatter(x = Xh %*% Wm_new_basis, 
                           y = Xh %*% Wm_opt, 
