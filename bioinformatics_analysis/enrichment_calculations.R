fastenrichment <- function(signature_ids,
                           gene_ids,
                           measurements,
                           enrichment_space = c("go_bp","go_cc","go_mf","kegg",
                                                "tf_dorothea","msig_db_h",
  ...
  if("go_cc" %in% enrichment_space) {
    genes <- factor(x = rep(1,nrow(measurements)),levels = c(0,1))
    names(genes) <- rownames(measurements)
    
    GOobject <- new("topGOdata",ontology = "CC", allGenes = genes, annot=annFUN.org, mapping="org.Hs.eg.db", 
                    ID = gene_id_type, nodeSize = 10)
    term.genes <- genesInTerm(GOobject, GOobject@graph@nodes)
    names(term.genes) <- paste0("FL1000_GO_CC_",names(term.genes))
    all_genesets <- c(all_genesets,term.genes)
  }
  if("go_mf" %in% enrichment_space) {
    genes <- factor(x = rep(1,nrow(measurements)),levels = c(0,1))
    names(genes) <- rownames(measurements)
    
    GOobject <- new("topGOdata",ontology = "MF", allGenes = genes, annot=annFUN.org, mapping="org.Hs.eg.db", 
                    ID = gene_id_type, nodeSize = 10)
    term.genes <- genesInTerm(GOobject, GOobject@graph@nodes)
    names(term.genes) <- paste0("FL1000_GO_MF_",names(term.genes))
    all_genesets <- c(all_genesets,term.genes)
  }
  ...
  NES_go_cc <- NES[sapply(rownames(NES),FUN=grepl,pattern="FL1000_GO_CC_"),]
  NES_go_mf <- NES[sapply(rownames(NES),FUN=grepl,pattern="FL1000_GO_MF_"),]
  ...
  pval_go_cc <- pval[sapply(rownames(pval),FUN=grepl,pattern="FL1000_GO_CC_"),]
  pval_go_mf <- pval[sapply(rownames(pval),FUN=grepl,pattern="FL1000_GO_MF_"),]
  ...
  all_NES <- list("NES GO BP" = NES_go_bp,"NES GO MF" = NES_go_mf,"NES GO CC" = NES_go_cc,
                  "NES KEGG" = NES_kegg,"NES TF" = NES_tf,
                  ...
                  "NES MSIG C7"=NES_c7)
  all_pval <- list("Pval GO BP" = pval_go_bp,"Pval GO MF" = pval_go_mf,"Pval GO CC" = pval_go_cc,
                   "Pval KEGG" = pval_kegg,"Pval TF"=pval_tf,
