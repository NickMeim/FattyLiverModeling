...
## Load dx 
mps_dx <- data.table::fread('../results/optimized_mps/dx_MPS_all.csv') %>% select(-V1)
wang_dx <- data.table::fread('../results/optimized_mps/dx_wang.csv') %>% select(-V1)
feaver_dx <- data.table::fread('../results/optimized_mps/dx_feaver.csv') %>% select(-V1)

## Load gene expression
Xmps <- data.table::fread('../preprocessing/X_B.csv') %>% column_to_rownames('V1')
Xwang <- data.table::fread('../preprocessing/X_wang.csv')
Xfeaver <- data.table::fread('../preprocessing/X_feaver.csv')

## Combine old and perturbed data
MPS <- rbind(as.matrix(Xmps),
             as.matrix(Xmps) + as.matrix(mps_dx))
Wang <- rbind(as.matrix(Xwang),
             as.matrix(Xwang) + as.matrix(wang_dx))
Feaver <- rbind(as.matrix(Xfeaver),
             as.matrix(Xfeaver) + as.matrix(feaver_dx))
